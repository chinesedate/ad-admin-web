  ...
  <div class="audit-tool-wrapper">
    ...
    <div class="audit-tool-list-content">
      <div class="audit-tool-list">
        ...
        <ul>
          <li class="audit-tool" v-for="auditIssue in auditArticleList" :key="auditIssue.id"
              @click="handleAuditArticleClick(auditIssue)">
            <div class="tool-content">{{auditIssue.title}}</div>
            <div class="tool-content"><p>{{auditIssue.description}}</p></div>
          </li>
        </ul>
      ...
      <div class="audit-issue-info">
        ...
        <div v-if="auditArticleList.length > 0">
          <p>
          ...
          <viewer ref="issueViewer" :initialValue="auditArticleInfo.text" height="500px"></viewer>
...
<script>
  ...
  import {fetchAuditIssue} from "@/api/issue";
  import {auditArticle, listAuditArticle} from "@/api/article";
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit() {
        auditArticle({
          id: this.auditArticleInfo.id, status: 1,
          title: this.auditArticleInfo.title,
      ...
      fetchAuditIssue(auditIssueId) {
        fetchAuditIssue(auditIssueId).then(res => {
          if (res.data.data != null) {
            this.auditArticleInfo = res.data.data;
            this.$refs.issueViewer.invoke('setMarkdown', this.auditArticleInfo.text);
          }
        });
      },
      handleAuditArticleClick(auditTool) {
        // 首先将列表中的简短信息赋值给待审核对象，然后通过接口查询详细信息后再更新
        this.auditArticleInfo = auditTool;
